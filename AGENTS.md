...
- The Anchor program implements `create_pot`, `join_pot`, `settle_pot`, `refund_pot`, `submit_proof`, and `set_profile` in `programs/accountability/src/lib.rs`.
...
- `submit_proof(uri)` lets the pot creator or any YES participant store a proof link, before or after the deadline until settlement. It rejects other signers (`UnauthorizedProof`), settled pots (`PotSettled`), blank links (`ProofRequired`), and links over 200 bytes (`ProofTooLong`).
- `create_pot` takes a trailing `access_hash: Option<[u8; 32]>` and `join_pot` a trailing `access_code: Option<String>` (64 bytes maximum, `AccessCodeTooLong`). The client hashes the invite code and the program stores that hash as given. A gated pot requires a code whose hash matches (`InvalidAccessCode`, also when the code is missing); an open pot ignores any supplied code. Settlement is unchanged. Hashing uses the `solana-sha256-hasher` crate because Anchor 0.32 does not re-export a hash module.
...
- The page passes `null` for the new `create_pot`/`join_pot` arguments and has no interface for proof links, invite codes, or profiles yet. Its pot list uses `fetchDecodablePots` in `src/lib/anchor/client.ts`, which decodes each account individually, instead of `program.account.pot.all()`, which fails the whole read when one account does not decode.
- The pot layout changed with these fields, and `POT_ACCOUNT_BYTES` in `src/lib/pot-values.ts` tracks it. Pots created by the earlier program build decode only when their account has spare space for the added fields; they then show no proof link and no invite code.
