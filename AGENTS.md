...
- The Anchor program implements `create_pot`, `join_pot`, `settle_pot`, and `refund_pot` in `programs/accountability/src/lib.rs`.
- Pots are PDAs derived from `['pot', creator, identifier]`. Each account stores its creator, judge, task (160 UTF-8 bytes maximum), fixed stake, deadline, creation time, YES/NO wallet lists, and settlement outcome. A settled pot with no outcome means timeout refunds; keep this layout compatible with older accounts. It has room for 10 total participants.
...
- The recovery rules and group challenge passed local browser rehearsal with three wallets, including an uninvolved caller refunding both stakes and opposite independent group verdicts. See `docs/recovery-verification.md`; repeat on devnet after upgrading.
