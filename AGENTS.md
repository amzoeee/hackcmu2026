...
- `src/app/api/demo-funds/route.ts` gives a low-balance demo wallet 0.25 devnet SOL. It uses the public devnet faucet by default or a server-only keypair configured with `SOLARA_DEMO_FUNDER_KEYPAIR`. The route refuses to run unless `NEXT_PUBLIC_SOLANA_NETWORK=devnet`; never configure it with a mainnet keypair.
- `.npmrc` sets `legacy-peer-deps=true`. `npm ci` fails with `ERESOLVE` without it, because `@privy-io/react-auth` pulls in EVM connectors whose transitive `@solana-program/*` packages pin `@solana/kit@^5` against the project's `@solana/kit@^8`.
...
- `npm run demo:check` reports the configured network, program deployment, deployer balance, funder reserve, and LAN URL. It exits unsuccessfully until the on-chain prerequisites are met.
...
- The local tests cover valid and invalid creation, duplicate, late, and full-pot joins, early/unauthorized/repeated settlement, payout-recipient validation, populated YES- and NO-side payouts, one-sided refunds, empty-pot settlement, and exact rent/dust preservation.
...
- The release profile is size-optimized and the program crate is `cdylib` only, enabling LTO. The deployable is 172,064 bytes and needs roughly 0.875 SOL in rent.
- The devnet program has not been deployed in this workspace. The configured deployer `6rahkcRv5q92wjP1zDUFpuZsYn8UXFJhVzpsngLjBghx` has 0 devnet SOL. Public RPC airdrops were rate-limited on 2026-09-12, and the proof-of-work faucet could not initialize an empty fee-payer. Fund the deployer with at least 0.9 SOL before running `npm run anchor:deploy:devnet`; use 1.5 SOL total if it will also fund two demo users.
