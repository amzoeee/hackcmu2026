...
- The devnet program has not been deployed in this workspace. The configured deployer `6rahkcRv5q92wjP1zDUFpuZsYn8UXFJhVzpsngLjBghx` has 0 devnet SOL, and the 224,288-byte build needs roughly 1.15 SOL to deploy. Public RPC airdrops were rate-limited on 2026-09-12, and the proof-of-work faucet could not initialize an empty fee-payer. Fund the deployer before running `npm run anchor:deploy:devnet`.
