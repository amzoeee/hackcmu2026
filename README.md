...
The committed program ID `EE5h4kXh8Pk2ECthCABpK7bLQ934n4TZjkRsuDgskYBb` is unchanged, but the pot layout and instruction set changed after the devnet deployment. Only the holder of the upgrade authority (the deployer key) can upgrade the devnet program, with `npm run anchor:deploy:devnet`; until then `submit_proof`, `set_profile`, and invite-code gating are unavailable there. If the upgrade reports the program account is too small, run `solana program extend EE5h4kXh8Pk2ECthCABpK7bLQ934n4TZjkRsuDgskYBb <bytes>` first. Pots created before the layout change decode only if their account has spare room for the new fields. A full one does not, and until it is repaired its stakes cannot be settled or refunded; any wallet can repair it by calling `resize_pot` and paying the rent for the added bytes.
